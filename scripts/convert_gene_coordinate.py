    ...
    for target in targetList:
        ...
        if prevchr != chr:
            ...
            if not os.path.isfile(ff):
                sys.stderr.write(
                    "WARNING: {} NOT FOUND (should be generated by splitByChromosome3). \
Likely due to no reads found in a targeted chromosome ({}), in turn likely due to currupted bam files\n".format(
                        ff, chr
                    )
                )
                # sys.exit(1)
...
def convertGeneCoordinate2(targetList, bufLocFolder):
    ...
    for target in targetList:
        ...
        if prevchr != chr:
            ...
            if not os.path.isfile(ff):
                sys.stderr.write(
                    "WARNING: %s NOT FOUND (should be generated by splitByChromosome3). \
        ...
        output_avg.write(
            "\t".join(
                [
                    ...
                    starts_leftmost,
                    ends[n],
                    str(target_ttl_readdepth / target_length),
                    str(target_length),
                ]
            )
            + "\n"
